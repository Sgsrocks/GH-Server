...
//import godzhell.Connection;
import godzhell.Server;
import godzhell.Config;
import godzhell.model.players.Player;
import godzhell.model.players.PlayerHandler;
//import godzhell.model.content.teleport.Position;
//import godzhell.model.content.teleport.TeleportExecutor;
...
public class PanelSettings {
	...
	public Player getPlayer(int id) {
		return  Server.playerHandler.players[id];
	}
		...
		if (p.ENTITY_LIST.getSelectedValue() != null) {
			...
			if (cmd.equalsIgnoreCase("Teleport Player")) {
				...
				if (area.equals("Custom Location")) {
					...
					if (tele != null) {
						c.getPA().startTeleport2(tele.getX(), tele.getY(), tele.getZ());
