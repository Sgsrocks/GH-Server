...
public class ShopAssistant {
	...
	public void resetShop(int ShopID) {
		...
		if (ShopID == Config.BOUNTY_HUNTER_STORE) {
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		if(c.myShopId == 16) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs " + ShopValue + " golden nuggets" + ShopAdd);
		...
		} else if(c.myShopId == Config.TZHAAR_MEJ_ROHS_RUNE_STORE) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs " + ShopValue + " tokkul" + ShopAdd);
	...
	if (Server.shopHandler.ShopSModifier[c.myShopId] > 1) {
		boolean IsIn = false;
		for (int i1 = 0; i1 <= ShopHandler.ShopItemsStandard[c.myShopId]; i1++) {
			if (itemID == (Server.shopHandler.ShopItems[c.myShopId][i1] - 1)) {
				IsIn = true;
				break;
			}
		}
	...
	if (amount > 0 && itemID == (c.playerItems[fromSlot] - 1)) {
		...
		if (c.myShopId == Config.PROSPECTOR_PERCYS_NUGGET_SHOP) {
		if(c.getItems().freeSlots() > 0 || c.getItems().playerHasItem(12012, 1)) {
			c.getItems().deleteItem2(itemID, amount);
			c.getItems().addItem(12012, price);
			...
			if (amount > 0) {
				...
				for (int i = amount; i > 0; i--) {
					TotPrice2 = (int) Math.floor(getItemShopValue(itemID, 0,
							fromSlot));
					TotPrice3 = (int) Math.floor(getItemShopValue(itemID, 0,
							fromSlot));
					...
					if (Slot1 == -1 && c.myShopId == Config.TZHAAR_HUR_LEKS_ORE_AND_GEM_STORE) {
						c.sendMessage("You don't have enough Tokkul.");
						break;
					}
					if (Slot1 == -1 && c.myShopId == Config.TZHAAR_MEJ_ROHS_RUNE_STORE ) {
						c.sendMessage("You don't have enough Tokkul.");
						break;
					}					
					...
					if (Slot2 == -1 && c.myShopId == Config.PROSPECTOR_PERCYS_NUGGET_SHOP) {
						c.sendMessage("You don't have enough Golden Nuggets.");
						break;
					}
					...
					if (c.myShopId == Config.PROSPECTOR_PERCYS_NUGGET_SHOP) {
						if (c.playerItemsN[Slot2] >= TotPrice3) {
							if (c.getItems().freeSlots() > 0) {
								if (Item.itemStackable[itemID] && c.getItems().playerHasItem(12012, TotPrice3*amount)) {
									if (Server.shopHandler.ShopItemsN[c.myShopId][fromSlot] < amount) {
										amount = Server.shopHandler.ShopItemsN[c.myShopId][fromSlot];
									}
									c.getItems().deleteItem(12012, c.getItems().getItemSlot(12012), TotPrice3*amount);
								...
								if (Item.itemStackable[itemID] && !c.getItems().playerHasItem(12012, TotPrice3*amount)) {
									int itemAmount = c.playerItemsN[c.getItems().getItemSlot(12012)]/TotPrice3;
									c.getItems().deleteItem(12012, c.getItems().getItemSlot(12012), TotPrice3*itemAmount);
								...
								c.getItems().deleteItem(12012, c.getItems().getItemSlot(12012),
										TotPrice3);
