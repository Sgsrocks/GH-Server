...
public class Potions {
	...
	public void handlePotion(int itemId, int slot) {
			if (itemId >= 20989 && itemId <= 20992) {
			    if (Boundary.isIn(c, Boundary.ENTIRERAIDS)) {
			    	if (itemId == 20989) {
			    		doOverload(itemId, -1, slot);
			    		return; //so return would end it right here? yes that was my mistake
			    		//the code kept running on as u saw it deleted for a second then it re appeared so now it will workwatch
			    	}
			        doOverload(itemId, itemId -1, slot);
			        System.out.println("should be drinking");
			    } else {
			    c.sendMessage("I should not attempt to drink these outside the Chambers of Xeric.");
			    }
			}
		...
		if (c.potionTimer.elapsed() > 1200) {
			...
			case 3046:
				...
				c.sendMessage("You quickly smash the empty vial using the trick a Barbarian taught you.");
				break;
			case 6685:
			...
			case 3030:
				...
				break;
			case 10925:
				drinkSuperRestorePot(itemId, 10927, slot); // sanfew serums
				curePoison(600);
			 	break;
			 case 10927:
				 drinkSuperRestorePot(itemId, 10929, slot);
			 	curePoison(600);
			 	break;
			 case 10929:
				 drinkSuperRestorePot(itemId, 10931, slot);
			 	curePoison(600);
			 	break;
			 case 10931:
				 drinkSuperRestorePot(itemId, -1, slot);
			...
			case 3022:
				drinkSuperEnergyPots(itemId, -1, slot);
				c.sendMessage("You quickly smash the empty vial using the trick a Barbarian taught you.");
				break;
	...
	public void curePoison(int duration) {
		c.getHealth().resolveStatus(HealthStatus.POISON, duration);
		c.getPA().sendGameTimer(ClientGameTimer.ANTIPOISON, TimeUnit.SECONDS, (int) (duration * .6));
		c.getPA().requestUpdates();
	}
	...
	public void drinkPrayerPot(int itemId, int replaceItem, int slot) {
		...
		if (SkillcapePerks.PRAYER.isWearing(c) || SkillcapePerks.isWearingMaxCape(c))
			c.playerLevel[5] += 5;
		c.playerLevel[5] += holyWrenchBonus(c);
		if (c.playerLevel[5] > c.getLevelForXP(c.playerXP[5]))
			c.playerLevel[5] = c.getLevelForXP(c.playerXP[5]);
			c.getPA().refreshSkill(5);
	}

	private int holyWrenchBonus(Player c) {
		int bonus = 0;
		if(c.getItems().playerHasItem(6714)) {
			c.sendMessage("Your holy wrench allows you to restore extra prayer.");
			if(c.getPA().getLevelForXP(c.playerXP[5]) >= 25) {
				bonus++;
			}
			if(c.getPA().getLevelForXP(c.playerXP[5]) >= 75) {
				bonus++;
			}
			//Formula wasnt rounding up, so i just did the math til 0.5 or higher^^^
			//bonus = (int) (c.getLevelForXP(c.playerXP[5]) * .02);
		}
		if(c.debugMessage) {
			c.sendMessage("[Debug] Pray lvl: " + c.getPA().getLevelForXP(c.playerXP[5]) + " bonus given: " + bonus);
		}
