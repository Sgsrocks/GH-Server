...
public class Boundary {
    ...
    public static boolean isIn(NPCSpawns npc, Boundary[] elfarea) {
        for (Boundary boundary : elfarea) {
            if (boundary.height >= 0) {
                if (npc.getHeight() != boundary.height) {
                    return false;
                }
            }
            if (npc.getXPos() >= boundary.minX && npc.getXPos() <= boundary.highX && npc.getYPos() >= boundary.minY && npc.getYPos() <= boundary.highY) {
                return true;
            }
        }
        return false;
    ...
    public static final Boundary MOTHERLODE_MINE = new Boundary(3648, 5569, 3838, 5727);
...
//public static final Boundary WILDERNESS = new Boundary(1486, 1444, 3767, 3730);
    ...
    //PORT LOCTIONS
    public static final Boundary KARAMJA_SHIPS = new Boundary(2948, 3142, 2961, 3160);
    public static final Boundary PORT_SARIM_SHIPS = new Boundary(3027, 3189, 3059, 3240);
    public static final Boundary BRIMHAVEN_SHIPS = new Boundary(2756, 3224, 2777, 3240);
    public static final Boundary CATHERBY_SHIPS = new Boundary(2788, 3407, 2809, 3431);
    public static final Boundary PORT_KHAZARD_SHIPS = new Boundary(2662, 3139, 2681, 3155);
    public static final Boundary CORSAIR_COVE_SHIPS = new Boundary(2566, 2832, 2592, 2859);
    public static final Boundary PORT_TYRAS_SHIPS = new Boundary(2137, 3115, 2151, 3127);
    public static final Boundary[] Hallowed_Sepulchre = new Boundary[] {
            new Boundary(2226, 5946, 2318, 6041),
            new Boundary(2479, 5939, 2581, 6028),
            new Boundary(2352, 5807, 2439, 5916),
            new Boundary(2480, 5799, 2585, 5907),
            new Boundary(2220, 5790, 2322, 5914)
    };
    public static final Boundary[] elfarea = new Boundary[] {
            new Boundary(2105, 3010, 2319, 3137),
            new Boundary(2104, 3137, 2345, 3465),
            new Boundary(2346, 3143, 2365, 3202)
    };
