...
import godzhell.model.npcs.NPCDumbPathFinder;
...
public class Test extends Command {
	...
	public void execute(Player player, String input) {
		...
		case "save":
			...
			case "hour":
				bxp.addTime(1440);
				System.out.println(bxp.EXPERIENCE_TIMER);
				System.out.println(Config.bonusXP);
				break;
			case "tp":
		...
		case "s":
			for (int i = 0; i < 7; i++) {
				...
				player.getPA().itemOnInterface(Config.NOT_SHAREABLE[random], i + 1, 33818, i);
				random = Misc.random(Config.NOT_SHAREABLE.length - 1);
				player.getPA().itemOnInterface(Config.NOT_SHAREABLE[random], i + 1, 33819, i);
				random = Misc.random(Config.NOT_SHAREABLE.length - 1);
				player.getPA().itemOnInterface(Config.NOT_SHAREABLE[random], i + 1, 33820, i);
				random = Misc.random(Config.NOT_SHAREABLE.length - 1);
				player.getPA().itemOnInterface(Config.NOT_SHAREABLE[random], i + 1, 33821, i);
			}
			break;
