...
import godzhell.model.content.achievement_diary.AchievementDiaryManager;
...
import godzhell.model.minigames.warriors_guild.WarriorsGuild;
import godzhell.model.miniquests.MageArena;
...
import godzhell.model.multiplayer_session.duel.Duel;
import godzhell.model.multiplayer_session.duel.DuelSession;
import godzhell.model.multiplayer_session.trade.Trade;
import godzhell.model.npcs.NPC;
import godzhell.model.npcs.NPCDeathTracker;
import godzhell.model.npcs.NPCHandler;
import godzhell.model.npcs.bosses.DemonicGorilla;
import godzhell.model.npcs.bosses.KalphiteQueen;
import godzhell.model.npcs.bosses.cerberus.Cerberus;
import godzhell.model.npcs.bosses.cerberus.CerberusLostItems;
import godzhell.model.npcs.bosses.skotizo.Skotizo;
import godzhell.model.npcs.bosses.skotizo.SkotizoLostItems;
...
import godzhell.model.npcs.bosses.zulrah.ZulrahLostItems;
import godzhell.model.npcs.instance.InstanceSoloFight;
import godzhell.model.npcs.pets.PetHandler;
import godzhell.model.npcs.pets.PetHandler.Pets;
import godzhell.model.players.combat.CombatAssistant;
import godzhell.model.players.combat.DamageQueueEvent;
...
import godzhell.net.outgoing.UnnecessaryPacketDropper;
import godzhell.util.Misc;
import godzhell.util.SimpleTimer;
...
public class Player extends Entity {
	...
	public int raidPoints, unfPotHerb, unfPotAmount, wrenchObject = 0, halloweenOrderNumber = 0, speed1 = -1,
			...
			roundNpc = 0, desertTreasure = 0, horrorFromDeep = 0, QuestPoints, teleGrabItem,
	...
	public void initialize() {
		try {
			...
			setSidebarInterface(13, 38161); //music
			QuestAssistant.sendStages(this);
			getMusic().load();
			//getMusic().updateRegionMusic(this.getRegionId());
	...
	if(Boundary.isIn(this, Boundary.KARUULM_SLAYER_DUNGEON)) {
		...
		if (inWild() && Boundary.isIn(this, Boundary.SAFEPK)) {
			...
			// getPA().walkableInterface(197);
			wildLevel = 54;
		} else if (Boundary.isIn(this, Boundary.MOTHERLODE_MINE)) {
			// getPA().walkableInterface(38843);
		...
		//} else if (Boundary.isIn(this, Boundary.Hallowed_Sepulchre)) {
			//getPA().walkableInterface(35698);
		} else {
			getPA().walkableInterface(-1);
			getPA().showOption(3, 0, "Null", 1);
		}
	...
	public int gertCat, restGhost,
			romeojuliet, runeMist, vampSlayer, cookAss, doricQuest, blackKnight, shieldArrav, witchspot,
			pirateTreasure, sheepShear, impsC, knightS, dragonSlayer, demonSlayer;

	public int bananas, ptjob, ChronicleCharges;

	public int cookingItem, cookingObject;
