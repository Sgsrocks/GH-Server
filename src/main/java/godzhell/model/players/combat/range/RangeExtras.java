package godzhell.model.players.combat.range;
...
import godzhell.model.players.combat.effects.bolts.DiamondBoltSpecial;
import godzhell.model.players.combat.effects.bolts.DragonBoltSpecial;
import godzhell.model.players.combat.effects.bolts.EmeraldBoltSpecial;
import godzhell.model.players.combat.effects.bolts.JadeBoltSpecial;
import godzhell.model.players.combat.effects.bolts.OnyxBoltSpecial;
import godzhell.model.players.combat.effects.bolts.OpalBoltSpecial;
import godzhell.model.players.combat.effects.bolts.PearlBoltSpecial;
import godzhell.model.players.combat.effects.bolts.RubyBoltSpecial;
import godzhell.model.players.combat.effects.bolts.SapphireBoltSpecial;
import godzhell.model.players.combat.effects.bolts.TopazBoltSpecial;
			...
			if (checkMultiChinchompaReqsNPC(npcId)) {
				...
				if (Misc.random(c.getCombat().calculateRangeAttack()) > Misc.random(NPCHandler.npcs[npcId].defence)) {
					...
					if (n.npcType == 7413) {
						...
						c.chinMultiDmg = c.chinMultiDmg + damage;
						n.appendDamage(c, damage, Hitmark.HIT);
						if(System.currentTimeMillis() - c.lastChinMulti >= 4000) {
							//c.sendMessage("NOW");
							AttackPlayer.addCombatXP(c, CombatType.RANGE, c.chinMultiDmg);
							//n.appendDamage(c, damage, Hitmark.HIT);
							c.lastChinMulti = System.currentTimeMillis();
							c.chinMultiDmg = 0;
						}
