				...
				if (gameMembers.get(i) != null) {
					if (Boundary.isIn(gameMembers.get(i), GAME_BOUNDARY)) {
						...
						if (player.pestControlDamage > MINIMUM_DAMAGE) {
							...
							int point_reward = Config.BONUS_PC && Config.BONUS_MINIGAME_WOGW ? 20 :
											   !Config.BONUS_PC && Config.BONUS_MINIGAME_WOGW ? 10 : 
											   Config.BONUS_PC && !Config.BONUS_MINIGAME_WOGW ? 15 : POINT_REWARD;
