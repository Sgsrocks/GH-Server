...
public class TeleportHandler {
	...
	String[] monsterNames = { "Select your Monster Teleport", "Rock Crabs", "Sand Crabs",  "Cows", "Yaks", "Bob's Island",
			"Desert Bandits", "Elf Warriors", "Dagannoths", "Slayer Tower", "Demonic Gorillas",
	...
	public void handleTeleports(Player player, int Id) {
		...
		case 180184: // Button 2
			if (!teleportCheck(player))
				return;
			if (player.teleSelected == 0) { // Monster - Sand crabs
				player.getPA().startTeleport(1792,3466,0, "modern", false);
		...
		case 180188:
			if (!teleportCheck(player))
				return;
			if (player.teleSelected == 0) { // Monster - cows
				player.getPA().startTeleport(3258,3270,0, "modern", false);
		...
		case 180191:
			...
			if (player.teleSelected == 0) { // Monsters - yaks
				player.getPA().startTeleport(2323,3796,0, "modern", false);
		...
		case 180194:
			if (!teleportCheck(player))
				return;
			if (player.teleSelected == 0) { // Monsters - bobs island
				player.getPA().startTeleport(2524,4771,0, "modern", false);
		...
		case 180197:
			...
			if (player.teleSelected == 0) { // Monsters - desert bandits
				player.getPA().startTeleport(3172,2983,0, "modern", false);
		...
		case 180200:
			...
			if (player.teleSelected == 0) { // Monsters - elf warriors
				player.getPA().startTeleport(2195,3245,0, "modern", false);
		...
		case 180203:
			if (!teleportCheck(player))
				return;
			if (player.teleSelected == 0) { // Monsters - dags
		...
		case 180212:
			if (!teleportCheck(player))
				return;
			if (player.teleSelected == 0) { // mosters teleport - demonic
		...
		case 180209:
			if (!teleportCheck(player))
				return;
			if (player.teleSelected == 0) { // Monsters - demonic
				player.getPA().startTeleport(2124,5660,0, "modern", false);
