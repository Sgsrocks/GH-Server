...
public class DiggingScrolls {
    ...
    public static enum DigData {
        ...
        DIG1(new String[]{"Dig near some giant mushrooms ", " behind the GrandTree"}, Items.CLUE_SCROLL_EASY_55, new Position(2458, 3504), 1),
    ...
    public static boolean loadClueInterface(Player player, int itemId) {
        ...
        for (int i = 0; i < digData.getHints().length; i++) {
            player.getPA().sendString(digData.getHints()[i], getChilds(digData.getHints())[i]);
        }
        return true;
    }

    public static boolean digClue(Player player) {
        DigData digData = DigData.forIdPosition(new Position(player.getPosition().getX(), player.getPosition().getY(), player.getPosition().getHeight()));
        if (digData == null) {
            return false;
        }
        if (!player.getItems().playerHasItem(digData.getClueId())) {
            return false;
        }
