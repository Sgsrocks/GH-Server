...
public class Desert {
	...
	public static void checkCactus(Player c, int objectId, int obX, int obY) {
		...
		Server.objectHandler.createAnObject(c, DRY_CACTUS, obX, obY, c.heightLevel, -1);
		for (int element[] : FILLS) {
			if (c.getItems().playerHasItem(element[0])) {
				c.getItems().deleteItem(element[0], c.getItems().getItemSlot(element[0]), 1);
			...
			if (itemId == element) {
				...
				CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
					...
					public void execute(CycleEventContainer container) {
								Server.objectHandler.createAnObject(c, objectId, obX, obY,  c.heightLevel,-1);
								container.stop();
							}
							@Override
							public void stop() {
				...
				c.sendMessage("You need a knife or a sharp weapon to cut this.");
			}
		}
	}
