package godzhell.model.content.skills.hunter;
...
import godzhell.model.content.skills.hunter.trap.Trap;
import godzhell.model.content.skills.hunter.trap.TrapProcessor;
import godzhell.model.content.skills.hunter.trap.TrapTask;
import godzhell.model.content.skills.hunter.trap.Trap.TrapState;
import godzhell.model.content.skills.hunter.trap.Trap.TrapType;
...
public final class Hunter {
	...
	public static boolean pickup(Player player, GlobalObject object) {
		...
		boolean issnare = trap.getObject().getObjectId() == 9375;
		boolean isboxtrap = trap.getObject().getObjectId() == 721;
		if (issnare) {
		player.startAnimation(5207);
		}
		if (isboxtrap) 		{
		player.startAnimation(5212);
		}
	...
	public static boolean claim(Player player, GlobalObject object) {
		...
		double percentOfXp = (trap.experience() * (player.getMode().getType().equals(ModeType.OSRS) ? 2 : Config.HUNTER_EXPERIENCE) / 100) * 2.5;
