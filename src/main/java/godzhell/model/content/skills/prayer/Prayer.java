package godzhell.model.content.skills.prayer;
...
public class Prayer {
	...
	public void bury(Bone bone) {
		...
		player.getPA().addSkillXP(bone.getExperience() * (Boundary.isIn(player, Boundary.LAVA_DRAGON_ISLE) && bone.getItemId() == 11943 ? (player.getMode().getType().equals(ModeType.OSRS) ? 4 * 16 : Config.PRAYER_EXPERIENCE * 4) : (player.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.PRAYER_EXPERIENCE)), Skill.PRAYER.getId(), true);
	...
	public void alter(final int amount) {
		...
		player.getPA().addSkillXP(bone.getExperience() * (player.getMode().getType().equals(ModeType.OSRS) ? (1 * 3) * 4 : Config.PRAYER_EXPERIENCE * 3), Skill.PRAYER.getId(), true);
		player.getItems().deleteItem2(bone.getItemId(), 1);
