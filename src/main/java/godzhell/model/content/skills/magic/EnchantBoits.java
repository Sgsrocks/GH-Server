package godzhell.model.content.skills.magic;
...
public static void UpdateItems(Player c) {
	...
	if(amountCosmic >= 1) {
	c.getPA().sendFrame126(" @gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountCosmic)+"/1", 35007);
	} else {
	c.getPA().sendFrame126(" @dre@0/1", 35007);
	}
	if(amountCosmic >= 1) {
	c.getPA().sendFrame126(" @gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountCosmic)+"/1", 35015);
	} else {
	c.getPA().sendFrame126(" @dre@0/1", 35015);
	}
	if(amountWater >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountWater)+"/1", 35040);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35040);
	}
	if(amountMind >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountMind)+"/1", 35038);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35038);
	}
	if(amountAir >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountAir)+"/2", 35028);
	} else {
	c.getPA().sendFrame126("@dre@0/2", 35028);
	}
	if(amountCosmic >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountCosmic)+"/1", 35009);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35009);
	}
	if(amountEarth >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountEarth)+"/2", 35032);
	} else {
	c.getPA().sendFrame126("@dre@0/2", 35032);
	}
	if(amountCosmic >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountCosmic)+"/1", 35011);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35011);
	}
	if(amountWater >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountWater)+"/2", 35034);
	} else {
	c.getPA().sendFrame126("@dre@0/2", 35034);
	}
	if(amountCosmic >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountCosmic)+"/1", 35017);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35017);
	}
	if(amountAir >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountAir)+"/3", 35046);
	} else {
	c.getPA().sendFrame126("@dre@0/3", 35046);
	}
	if(amountNat >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountNat)+"/1", 35054);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35054);
	}
	if(amountCosmic >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountCosmic)+"/1", 35013);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35013);
	}
	if(amountFire >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountFire)+"/2", 35036);
	} else {
	c.getPA().sendFrame126("@dre@0/2", 35036);
	}
	if(amountCosmic >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountCosmic)+"/1", 35019);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35019);
	}
	if(amountFire >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountFire)+"/5", 35044);
	} else {
	c.getPA().sendFrame126("@dre@0/5", 35044);
	}
	if(amountBlood >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountBlood)+"/1", 35042);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35042);
	}
	if(amountCosmic >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountCosmic)+"/1", 35021);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35021);
	}
	if(amountEarth >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountEarth)+"/10", 35067);
	} else {
	c.getPA().sendFrame126("@dre@0/10", 35067);
	}
	if(amountLaw >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountLaw)+"/2", 35048);
	} else {
	c.getPA().sendFrame126("@dre@0/2", 35048);
	}
	if(amountCosmic >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountCosmic)+"/1", 35023);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35023);
	}
	if(amountEarth >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountEarth)+"/15", 35030);
	} else {
	c.getPA().sendFrame126("@dre@0/15", 35030);
	}
	if(amountSoul >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountSoul)+"/1", 35052);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35052);
	}
	if(amountCosmic >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountCosmic)+"/1", 35025);
	} else {
	c.getPA().sendFrame126("@dre@0/1", 35025);
	}
	if(amountFire >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountFire)+"/20", 35069);
	} else {
	c.getPA().sendFrame126("@dre@0/20", 35069);
	}
	if(amountDeath >= 1) {
	c.getPA().sendFrame126("@gre@"+godzhell.util.Misc.getValueWithoutRepresentation(amountDeath)+"/1", 35050);
	} else {
