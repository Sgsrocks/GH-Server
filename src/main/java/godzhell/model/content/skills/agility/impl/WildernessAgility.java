	...
	public boolean wildernessCourse(final Player c, final int objectId) {
		...
		case 23555:
			if (c.getAgilityHandler().hotSpot(c, 2998, 3916)) {
				...
				c.getPA().addSkillXP(40 * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.AGILITY_EXPERIENCE), c.playerAgility, true);
			}
		...
		case WILDERNESS_ROCKS_OBJECT:
			...
			c.getAgilityHandler().lapFinished(c, 5, c.getMode().getType().equals(ModeType.OSRS) ? 2284 : 15600, 6000);
