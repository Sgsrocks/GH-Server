	...
	public void execute() {
		switch (getNext()) {
			case 0:
				if (player.getSlayer().getTask().isPresent()) {
					if (player.getSlayer().getTask().get().getLevel() > player.playerLevel[Skill.SLAYER.getId()]) {
						DialogueManager.sendNpcChat(getPlayer(), 405, Emotion.ANNOYED, "You have been assigned a task you cannot complete.",
			...
			case 5:
				...
				Optional<SlayerMaster> master = SlayerMaster.get(405);
				...
				if (task.isPresent() && master.isPresent() && myMaster.isPresent()) {
					if (getPlayer().getSlayer().getMaster() != master.get().getId()) {
						...
						DialogueManager.sendNpcChat(getPlayer(), 405, Emotion.CALM, "You currently have " + player.getSlayer().getTaskAmount() + " "
					...
					player.getSlayer().createNewTask(405);
				}
				break;
			case 6:
				break;
