	...
	public void execute() {
		switch (getNext()) {
			case 0:
				if (player.getSlayer().getTask().isPresent()) {
					if (player.getSlayer().getTask().get().getLevel() > player.playerLevel[Skill.SLAYER.getId()]) {
						DialogueManager.sendNpcChat(getPlayer(), 402, Emotion.ANNOYED, "You have been assigned a task you cannot complete.",
				...
				DialogueManager.sendNpcChat(getPlayer(), 402, Emotion.CALM, "What do you want?");
				setNext(1);
				break;
			case 1:
			...
			case 5:
				...
				Optional<SlayerMaster> master = SlayerMaster.get(402);
				...
				if (task.isPresent() && master.isPresent() && myMaster.isPresent()) {
					if (getPlayer().getSlayer().getMaster() != master.get().getId()) {
						if (myMaster.get().getLevel() < master.get().getLevel()) {
							DialogueManager.sendNpcChat(getPlayer(), 402, Emotion.CALM, "You have an easier task from a different master.",
						...
						DialogueManager.sendNpcChat(getPlayer(), 402, Emotion.CALM, "I can give you an easier task but this will reset your",
								"consecutive tasks completed if you have an active task.",
								"Are you sure this is what you want to do?");
						getPlayer().sendMessage("@blu@test2");
						setNext(6);
					} else {
						DialogueManager.sendNpcChat(getPlayer(), 402, Emotion.CALM, "You currently have " + player.getSlayer().getTaskAmount() + " "
					...
					player.getSlayer().createNewTask(402);
				}
				break;
			case 6:
				break;
	...
	public boolean clickButton(int id) {
		switch(id) {
			...
			case DialogueConstants.OPTIONS_2_1:
				...
				if (getPlayer().getSlayer().getMaster() != master_npc.get().getId() && master_npc.get().getId() != 402) {
					DialogueManager.sendNpcChat(getPlayer(), 402, Emotion.CALM, "You already seem to have an active task with someone else.");
					setNext(2);
					return false;
				}
