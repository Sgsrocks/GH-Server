package godzhell.cache.index;

import java.io.IOException;
import java.nio.ByteBuffer;

import godzhell.cache.Archive;
...
public class IndexTable {
	...
	private void initObjectDefIndices(Archive configArchive) throws IOException {
		...
		int objectCount = buf.getShort();
		System.out.println(String.format("Loaded: %d objects", objectCount));
	...
	private void initMapIndices(Archive versionListArchive) throws IOException {
		...
		int indices = buf.getShort() & 0xFFFF;
		System.out.println(String.format("Loaded: %d maps", indices));
		...
		for (int i = 0; i < indices; i++) {
			...
			MapIndex index = new MapIndex(area, mapFile, landscapeFile);
			mapIndices[i] = index;
		}
	}
