package ethos.model.content.skills.slayer;
			...
			if (name.equals(task.getPrimaryName()) || ArrayUtils.contains(task.getNames(), name)) {
				...
				master.ifPresent(m -> {
					...
					case 6797:
						...
						: task.getExperience() * (player.getMode().getType().equals(ModeType.OSRS) ? 6 : Config.SLAYER_EXPERIENCE),
					...
					if (taskAmount == 0) {
						...
						player.sendMessage("You have been given "+(m.getPointReward(0) + bonusPoints)+" total Slayer points for completing this Slayer task!");
					}
				});
			}
		});
	...
	public void handleSuperiorExp(NPC npc) {
		task.ifPresent(task -> {
			...
			if (!name.equals(task.getPrimaryName())) {
				...
				if (isSuperiorNpc()) {
					player.getPA().addSkillXP(player.getRechargeItems().hasAnyItem(13113, 13114, 13115) && Boundary.isIn(player, Boundary.SLAYER_TOWER_BOUNDARY) ? (int) (task.getExperience() * 1.10) * (player.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.SLAYER_EXPERIENCE)
					: task.getExperience() * 10 * (player.getMode().getType().equals(ModeType.OSRS) ? 6 : Config.SLAYER_EXPERIENCE),
					Skill.SLAYER.getId(), true);
...
//			updatePoints();
		...
		case 162043:
			...
			if (points < 75) {
				player.sendMessage("You need 75 slayer points to learn this.");
				return true;
			}
			points -= 75;
