package ethos.model.content.skills.crafting;
...
public class Crafting {
		...
		for (int j = 0; j < gems.length; j++) {
			if (gems[j][0] == id) {
				if (c.playerLevel[Player.playerCrafting] >= gems[j][2]) {
					...
					c.getPA().addSkillXP(gems[j][3] * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.CRAFTING_EXPERIENCE), Player.playerCrafting, true);
					if (gems[j][0] == 1631)
	...
	public void craftHides(int id) {
		for (int j = 0; j < amount; j++) {
			...
			c.getPA().addSkillXP(exp * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.CRAFTING_EXPERIENCE), Player.playerCrafting, true);
		}
		resetCrafting();
	}
			...
			Server.getEventHandler().submit(new Event<Player>("skilling", c, 2) {
				...
				public void execute() {
					...
					if (attachment.getItems().playerHasItem(g.uncut)) {
						...
						attachment.getPA().addSkillXP(g.experience * (attachment.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.CRAFTING_EXPERIENCE), Skill.CRAFTING.getId(), true);
