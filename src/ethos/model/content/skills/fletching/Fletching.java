package ethos.model.content.skills.fletching;
...
public class Fletching {
	...
	public void fletchGem(int use, int used) {
		...
		gem.ifPresent(g -> {
			...
			player.getPA().addSkillXP(g.getExperience() * (player.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.FLETCHING_EXPERIENCE), Skill.FLETCHING.getId(), true);
		});
	}
	...
	public void fletchUnfinishedBolt(int boltId) {
		...
		bolt.ifPresent(b -> {
			...
			player.getPA().addSkillXP(b.getExperience() * (player.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.FLETCHING_EXPERIENCE), Skill.FLETCHING.getId(), true);
		});
	}
	...
	public void fletchHeadlessArrows() {
		...
		player.getPA().addSkillXP(15 * (player.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.FLETCHING_EXPERIENCE), Skill.FLETCHING.getId(), true);
	}
	...
	public void fletchArrow(int arrowId) {
		...
		arrow.ifPresent(a -> {
			...
			player.getPA().addSkillXP((int) a.getExperience() * (player.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.FLETCHING_EXPERIENCE), Skill.FLETCHING.getId(), true);
		});
	}
	...
	public void fletchDart(int dartId) {
		...
		dart.ifPresent(d -> {
			...
			player.getPA().addSkillXP((int) (10 * d.getExperience()) * (player.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.FLETCHING_EXPERIENCE), Skill.FLETCHING.getId(), true);
		});
	}
