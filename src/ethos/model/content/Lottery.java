...
public class Lottery {
	...
	public static void startLottery() {
		...
		for (int j = 0; j < PlayerHandler.players.length; j++) {
			if (PlayerHandler.players[j] != null) {
				...
				c2.sendMessage("@blu@Saintzscape's Lottery has been started. Talk to the gambler to enter. Good luck!");
			}
		}
	}
	...
	public static void winLottery(int iD){
		...
		for (int j = 0; j < PlayerHandler.players.length; j++) {
			if (PlayerHandler.players[j] != null) {
				Player c2 = (Player)PlayerHandler.players[j];
				c2.sendMessage("Congratulations " + c2.playerName + " for winning the Saintz Jackpot! Enjoy your @blu@ " + lotteryAmount + " coins.");
			}
		}
		resetLottery();
	}
