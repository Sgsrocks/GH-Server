...
public class Smelting {
	...
	public enum Bars {
		...
		IRON(440, 453, 2351, 15, 13, 2406, false, "iron"), 
		SILVER(442, -1, 2355, 20, 14, 2407, false, "silver"), 
		STEEL(440, -453, 2353, 30, 18, 2409, true, "steel"), 
		GOLD(444, -1, 2357, 40, 23, 2410, false, "gold"), 
		MITHRIL(447, -1, 2359, 50, 30, 2411, true, "mithril"), 
		ADAMANT(449, -1, 2361, 70, 38, 2412, true, "adamant"), 
		RUNE(451, -1, 2363, 85, 50, 2413, true, "rune");
	...
	private static void appendDelay(Player c, String usage) {
		...
		double percentOfXp = c.bar.getExp() * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.SMITHING_EXPERIENCE) / 2;
		if (c.smeltAmount > 0) {
			boolean hasItems;
			if (c.bar.twoOres())
				hasItems = hasItems(c, c.bar.getOre1(), c.bar.getOre2(), c.bar.getBar());
			...
			if (hasItems) {
				...
				if (c.bar.twoOres()) {
					...
					c.getPA().addSkillXP((int) (Objects.equals(usage, "INFERNAL") ? c.bar.getExp() * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.SMITHING_EXPERIENCE / 2) : c.bar.getExp() * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.SMITHING_EXPERIENCE) + (goldSmithGaunts ? percentOfXp : 0)), Player.playerSmithing, true);
				} else {
					if (!Objects.equals(usage, "INFERNAL")) {
						c.startAnimation(899);
					}
					...
					if ((c.bar.getBar() == 2351 && Misc.random(100) >= 50) && c.playerEquipment[c.playerRing] != 2568)
						...
						c.getItems().addItem(c.bar.getBar(), 1);
						c.getPA().addSkillXP((int) (usage == "INFERNAL" ? c.bar.getExp() * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.SMITHING_EXPERIENCE / 2) : c.bar.getExp() * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.SMITHING_EXPERIENCE) + (goldSmithGaunts ? percentOfXp : 0)), Player.playerSmithing, true);
					}
				}
			}
		} else {
