	...
	public static void doAction(final Player c, final int toadd, final int toremove, final int toremove2, final int timestomake, final int NOTUSED, final int NOTUSED2,
		...
		Server.getEventHandler().submit(new Event<Player>("skilling", c, 3) {
			...
			public void execute() {
						...
						if (ItemAssistant.getItemName(toadd).contains("arrow")) {
							...
							if (ItemAssistant.getItemName(toadd).contains("dart")) {
								...
								if (ItemAssistant.getItemName(toadd).contains("knife")) {
									if (Boundary.isIn(c, Boundary.VARROCK_BOUNDARY)) {
										if (ItemAssistant.getItemName(toadd).contains("Steel")) {
											c.getDiaryManager().getVarrockDiary().progress(VarrockDiaryEntry.SMITH_STEEL_KNIFES, true);
										}
										if (ItemAssistant.getItemName(toadd).contains("Rune")) {
											c.getDiaryManager().getVarrockDiary().progress(VarrockDiaryEntry.SMITH_RUNE_KNIFES, true);
										}
									}
									//do diary check here?
									c.getItems().addItem(toadd, 5);
				...
				c.getPA().addSkillXP((int) (xp * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.SMITHING_EXPERIENCE)), 13, true);
	...
	public void smelt(final int barType) {
		if (c.smeltAmount > 0) {
			...
			if (hasOres(barType)) {
				...
				c.getPA().addSkillXP(exp * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.SMITHING_EXPERIENCE), Player.playerSmithing, true);
